...
export default {
  ...
  axe: {
    ...
    // Pages to test
    testPages: ['/', '/about', '/contact', '/services', '/portfolio', '/blog', '/privacy', '/terms'],
  ...
  lighthouse: {
    ...
    accessibility: {
      ...
      customAudits: ['skip-link-focus', 'aria-live-region-functionality', 'focus-management', 'keyboard-navigation'],
    },
  ...
  workflow: {
    // When to run tests
    triggers: ['pre-commit', 'pre-push', 'pull-request', 'deployment', 'scheduled'],
  ...
  customChecks: {
    ...
    focusManagement: {
      enabled: true,
      checks: ['skip-links-functional', 'focus-trap-working', 'focus-restoration', 'keyboard-navigation-complete'],
    },

    // ARIA live regions verification
    liveRegions: {
      enabled: true,
      checks: ['live-region-announcements', 'polite-vs-assertive-usage', 'aria-atomic-proper-usage'],
    },
    ...
    motionAccessibility: {
      enabled: true,
      checks: ['prefers-reduced-motion-respected', 'animation-duration-limits', 'motion-triggered-interactions'],
    },
  },
};
