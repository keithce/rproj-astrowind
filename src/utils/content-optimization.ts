...
export const CONTENT_TEMPLATES = {
  serviceHero: {
    ...
    example: 'Professional Audio Mixing: Get Radio-Ready Sound. Turn your rough tracks into polished hits. Perfect for independent artists and producers.'
  },
  ...
  testimonial: {
    ...
    example: 'My workflow time cut in half. I finally feel organized and in control. I recommend Keith to any creative professional.'
  }
};
  ...
  longSentences: [
    {
      ...
      fix: (match, p1, conjunction, p3) => `${p1}. ${conjunction === 'and' ? 'Also, ' : conjunction === 'but' ? 'However, ' : ''}${p3.charAt(0).toUpperCase()}${p3.slice(1)}`
    }
  ],
  ...
  passiveVoice: [
    {
      ...
      fix: (match, subject, verb, agent) => `${agent} ${verb.replace('ed', '')} ${subject}`
    }
  ],
    ...
    .map(sentence => {
      ...
      if (words.length > maxLength) {
        ...
        const breakIndex = words.findIndex((word, i) => 
          i > 5 && conjunctions.includes(word.toLowerCase())
        );
  ...
  const patterns = [
    {
      ...
      replacement: (match, subject, verb, agent, punctuation) => {
